...
export const updateMasterConfig = async (req, res) => {
    try {
        ...
        for (const [field, files] of Object.entries(imageMap)) {
            for (const file of files) {
                ...
                if (field === 'tiles') {
                    ...
                    if (
                        !isNaN(tileIndex) &&
                        updatedFields.tiles_info.tiles[tileIndex]
                    ) {
                        await deleteFromS3(
                            updatedFields.tiles_info.tiles[tileIndex]
                        );
                ...
                // Slider image update by index
                    ...
                    if (
                        !isNaN(sliderIndex) &&
                        updatedFields.slider_images?.[sliderIndex]
                    ) {
                        await deleteFromS3(
                            updatedFields.slider_images[sliderIndex]
                        );
                ...
                // Room/area images (any dynamic key)
                    ...
                    !['tiles', 'slider_image', 'feature_images'].includes(field)
                ) {
        ...
        if (title || description || features) {
            updatedFields.tiles_info.title =
                title || updatedFields.tiles_info.title;
            updatedFields.tiles_info.description =
                description || updatedFields.tiles_info.description;
            if (features) {
                try {
                    ...
                    updatedFields.tiles_info.features = features
                        .split(',')
                        .map((f) => f.trim());
                }
            }
        ...
        updatedFields.contact_info = {
            ...
            socialMediaURL:
                socialMediaURL || updatedFields.contact_info.socialMediaURL,
        };
...
export const getAllConfigs = async (req, res) => {
    try {
        const { id } = req.params;
        const configs = await MasterConfig.find({ created_by: id }).sort({
            createdAt: -1,
        });
        res.status(200).json({
            success: true,
